...
# BASIC APP CONFIG
SECRET_KEY = os.environ.get('PDA_SECRET_KEY')
LOG_LEVEL = os.environ.get('PDA_LOG_LEVEL', "DEBUG")
LOG_FILE = os.environ.get('PDA_LOG_FILE', None)

# TIMEOUT - for large zones
TIMEOUT = os.environ.get('PDA_TIMEOUT', 10)

# UPLOAD DIR
UPLOAD_DIR = os.path.join(basedir, 'upload')

# DATABASE CONFIG
SQLA_DB_DRIVER = os.environ.get('PDA_DB_DRIVER', "mysql")
SQLA_DB_USER = os.environ.get('PDA_DB_USER')
SQLA_DB_PASSWORD = os.environ.get('PDA_DB_PASSWORD')
SQLA_DB_HOST = os.environ.get('PDA_DB_HOST')
SQLA_DB_PORT = os.environ.get('PDA_DB_PORT', 3306)
SQLA_DB_NAME = os.environ.get('PDA_DB_NAME')
SQLALCHEMY_TRACK_MODIFICATIONS = True

SQLALCHEMY_DATABASE_URI = '{driver}://{user}:{password}@{host}:{port}/{dbname}'.format(
    driver=SQLA_DB_DRIVER,
    user=SQLA_DB_USER,
    password=SQLA_DB_PASSWORD,
    host=SQLA_DB_HOST,
    port=SQLA_DB_PORT,
    dbname=SQLA_DB_NAME)
